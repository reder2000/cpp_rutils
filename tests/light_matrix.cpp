...
TEST_CASE("light_matrix", "[name][hide]")
{
  ...
  CHECK(mult(aa, 2) == aa2);
  CHECK(is_equal(m3.get_row_view(1), std::vector<int>{3, 4}));
  m3.assign_col(1, std::vector<int>{7, 8, 9});
  CHECK(is_equal(m3.get_row_view(1), std::vector<int>{3, 8}));
  mult_self(aa, aa2);
  CHECK(aa == std::vector<int>{2, 8, 18});
}
