...
#define name_tuple_sequence(name) \
	BOOST_PP_TUPLE_TO_SEQ(name_tuple(name))
...
#define  name_tuple_struct_vector_declare(name) \
	struct BOOST_PP_CAT(name,_vector) {  \
		...
		BOOST_PP_CAT(name,_vector)(size_t n) : BOOST_PP_SEQ_FOR_EACH_I(struct_declare_vector_init_elem,_, name_tuple_sequence(name)) {} \
		size_t size() const { \
				return BOOST_PP_CAT(_, BOOST_PP_TUPLE_ELEM(1, BOOST_PP_SEQ_ELEM(0, name_tuple_sequence(name)))).size(); 		} \
	}
...
#define  name_tuple_struct_vector_merge_elem(r,data,elem) \
	res.BOOST_PP_CAT(_,BOOST_PP_TUPLE_ELEM(1,elem)) = merge( v.BOOST_PP_CAT(_,BOOST_PP_TUPLE_ELEM(1,elem))) ;
...
{
	size_t total_size = std::accumulate(vs.begin(), vs.end(), (size_t)0, [](size_t i, auto const& t) {return i + t.size(); });
	std::vector<T> res;
