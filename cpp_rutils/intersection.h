...
std::vector<typename In::value_type> intersection_vc(const In& v1, const In& v2, Compare comp)
{
  ...
  std::set_intersection(v1.begin(), v1.end(), v2.begin(), v2.end(), std::back_inserter(res), comp);
  return res;
}
...
std::vector<typename std::invoke_result_t<Fun, typename In::value_type>>
intersection_vc_fun(const In& v1, const In& v2, Fun fun, Compare comp)
{
...
std::vector<typename std::invoke_result_t<Fun, typename In::value_type>>
intersection_vc_fun(const In& v1, const In& v2, Fun fun)
{
...
template <class In, class... Args>
std::vector<typename In::value_type> intersection_vcs(const In& v1,
                                                      const In& v2,
                                                      const Args... args)
{
...
std::vector<T> set_intersection(const std::vector<std::reference_wrapper<const std::vector<T>>>& vs)
{
  ...
  const size_t M =
      std::accumulate(vs.begin(), vs.end(), size_t(0),
                      [](size_t a, const std::reference_wrapper<const std::vector<T>>& b)
