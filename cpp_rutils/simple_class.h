...
#define EMIT_SIMPLE_CLASS_PYBIND11(name, m)                                                       \
  py::class_<name>(                                                                               \
      ...
      BOOST_PP_STRINGIZE(name))                                                                   \
          .def(py::init(                                                                          \
              [](BOOST_PP_LIST_FOR_EACH_I(EMIT_SIMPLE_CLASS_PYBIND11_DECLARE, name,               \
                                          BOOST_PP_TUPLE_TO_LIST(NAME_TUPLE(name))))              \
              {                                                                                   \
                return name{BOOST_PP_LIST_FOR_EACH_I(EMIT_SIMPLE_CLASS_PYBIND11_LIST_NAMES, name, \
                                                     BOOST_PP_TUPLE_TO_LIST(NAME_TUPLE(name)))};  \
              })) BOOST_PP_LIST_FOR_EACH_I(EMIT_SIMPLE_CLASS_PYBIND11_PROPERTIES, name,           \
                                           BOOST_PP_TUPLE_TO_LIST(NAME_TUPLE(name)))
  ...
  py::class_<nmspc::name>(                                                                   \
      m, BOOST_PP_STRINGIZE(name))                                                           \
             .def(py::init(                                                                  \
                 ...
                 {                                                                           \
                   return nmspc::name{                                                       \
                       BOOST_PP_LIST_FOR_EACH_I(EMIT_SIMPLE_CLASS_PYBIND11_LIST_NAMES, name, \
                                                BOOST_PP_TUPLE_TO_LIST(NAME_TUPLE(name)))};  \
                 }))                                                                         \
                 BOOST_PP_LIST_FOR_EACH_I(EMIT_SIMPLE_CLASS_PYBIND11_NPSPC_PROPERTIES,       \
                                          (nmspc, name), BOOST_PP_TUPLE_TO_LIST(NAME_TUPLE(name)))
